...
def meu_algoritmo_construtivo(distancias_apsp, capacidade_veiculo, depot_idx,
    ...
    while servicos_pendentes:
        loop_count_outer += 1
        # if loop_count_outer > len(lista_inicial_servicos) + 5 and loop_count_outer > 20: 
        ...
        while True: # Loop para construir a rota atual
            ...
            for k, s_p in enumerate(servicos_pendentes):
                if s_p["id_output"] == servico_escolhido["id_output"]: 
                    idx_para_remover = k
                    break
        ...
        if servicos_adicionados_nesta_iteracao_rota > 0: 
            ...
            if custo_retorno_depot != math.inf:
                ...
                pass # Silencia o alerta para rotas inviáveis se não for um erro crítico
            ...
            if not servicos_pendentes: 
                pass # Se não há mais serviços pendentes, o loop principal vai terminar
